...
jQuery(document).ready( function(){
	...
	// Check if faq hash is present and open it
	if(typeof location.hash != 'undefined') {
		if(/^#faq-.+/.test(location.hash)) {
			jQuery(location.hash).children('.arconix-faq-title').removeClass('faq-closed').addClass('faq-open')
			jQuery(location.hash).children('.arconix-faq-content').removeClass('faq-closed').addClass('faq-open').slideToggle();
		}
	}
});